...
% Proper enumeration - the enumitem does not work here?
...
\begin{versionhistory}
  \vhEntry{0.1}{29.10.2017}{A. Abreu}{First Data Model. Initial Definition}
  \vhEntry{1.0}{09.03.2018}{C. Kirsch}{First full draft, including algorithms and Data Model.}
\end{versionhistory}
...
\begin{figure}
  \centering
  \includegraphics{images/extraction/bam_flowchart}
...
\end{tabular}}
\caption{CosmicObservation table header version \dmVersion}
\end{table}
...
TRACK\_LEN & Estimated incident particle track length & $\mu$m & Float \\ \hline
TRACK\_LEN\_ERR & Uncertainty in the estimated incident particle track length & $\mu$m & Float \\ \hline
TRACK\_THETA & Estimated incident particle impact angle & deg & Float \\ \hline
TRACK\_THETA\_ERR & Uncertainty in the estimated incident particle impact angle & deg & Float \\ \hline
\end{tabular}}
\caption{CosmicObservation table version \dmVersion}
\end{table}

\subsection{Flux Dataset Description}
\label{sec:FluxModel}
We provide additional files listing just the measured flux per observation, sorted by the source observation type and chip. A file has been generated for each source type whose extraction has been defined above, as well as for the PPE counters (named SM-PPE).
...
\hline
{\textit{\textbf{Name}}} & {\textit{\textbf{Description}}} & {\textit{\textbf{Unit}}} & {\textit{\textbf{Type}}} \\ \hline
COUNTS & Number of particles in the corresponding observation & NA & Integer \\ \hline
FLUX & Estimated particle flux & particles\,cm$^{-2}$\,s$^{-1}$ & Float \\ \hline
FLUX\_ERR & Poisson Uncertainty of estimated particle flux & particles\,cm$^{-2}$\,s$^{-1}$ & Float \\ \hline
TIME & Source acquisition time & MJD & Float \\ \hline
DATE & Source acquisition time & UTC & String \\ \hline
...
\bibitem{GAIA-CA-TN-ESAC-AAA-035-1} A. Abreu \emph{Radiation Environment and Cosmics from Gaia at L2}.
