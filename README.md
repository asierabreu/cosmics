...
This anti-correlation has also been observed in Gaia when analyzing the cosmic ray hit counts produced by Gaia telemetry along the mission timeline since the launch in Dec 2013 (at Solar maximum). Gaia onboard detection algorithms, have to autonomously detect objects (stars) and allocate resources for further observation of these along their movement across the focal plane as Gaia rotates. That detection happens at the SkyMapper CCDs, where objects failing to meat certain criteria are also rejected for further processing. Amongst those rejected objects are the Cosmic Rays. This information is telemered down to Earth in the form of detections chain statistics, amongst which are the Prompt Particle Event Counters or PPE counters.   

The following is a long term plot of the Prompt Particle Event counters (PPE counters) at L2 generated by the VPU detection algorithms onboard Gaia, that shows an increasing level of Galactic Cosmic ray hits (marked by the increasing baseline) on top of which solar particle events shows as peaks, as we progress onto solar minimum, i.e, decrease solar activity.  Note that that PPE counters are a by-product of the onboard autonomous detection chain and hence a 'proxy' for cosmic ray counts not a direct measurement.S

Prompt Particle Events Evolution with OBMT:

<img src="images/ppes-sunspot.png" width="700" height="500" align="centre">
