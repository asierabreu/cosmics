...
def process_BAM_OBS(pathgroup, write_grouping, root_dir):
    """
    Extracts and writes TrackObservations from a pathgroup, and writes them to fits files

    pathgroup: A tuple of a list of gbin paths grouped in time, and a single path indicating the "next" path. If there is no next path, it should be an empty string
    ...
    # get the paths
    pathlist = pathgroup[0]
    npaths = len(pathlist)
    nextpath = pathgroup[1]
    ...
    while boxcar1.nfilled < boxcar1.npatterns and boxcar2.nfilled < boxcar2.npatterns:
        try:
            obs = reader.__next__()
            ...
            reader.close()
            ### output this file
            write_badfile(pathlist[i_path], root_dir)
            # get the next or stop
            i_path += 1
            if i_path >= npaths:
                break  # we're done with the pathlist
            else:
        ...
        if obs.fov==0:
            if boxcar1.nfilled < boxcar1.npatterns:
                ...
                # this boxcar is full, save this for later
                buffer1.append(obs)
        elif obs.fov==1:
    ...
    # prepare boxcars for first extraction
    boxcar1.i_sig = boxcar1.boxrad
    boxcar2.i_sig = boxcar2.boxrad
    ...
    # then the next path, for only the first few BamObs
    # if the next path is empty, just one more iteration at the end
    if nextpath=='':
        pass
    else:
        # use the buffers again: First fill them from the file as far as we need them
        reader = gbin_reader.GbinReader(nextpath)
        while (len(buffer1) < 2*boxrad) and (len(buffer2) < 2*boxrad):
            try:
                obs = reader.__next__()
            except StopIteration:
                # output this file
                write_badfile(nextpath, root_dir)
                break
            if obs.fov==0:
                if len(buffer1) < 2*boxrad:
                    buffer1.append(obs)
            elif obs.fov==1:
                if len(buffer2) < 2*boxrad:
                    buffer2.append(obs)

        # then extract using the buffers
        while len(buffer1)>0:
            try:
                outlist1.append(BoxCar_extract_TrackObs(boxcar1,gain,readnoise))
                pattern,acqTime = decode_BamObs(buffer1.pop(0),bias,gain)
