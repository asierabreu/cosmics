...
def process(datepathlist, calibdat, root_dir):
    ...
    print('writing to', outpath+'/'+outname)
    pfuncs.write_Obslist(obslist, outpath+'/'+outname)
    
    t2 = time.time()
    print("writing:", str(t2-t1))
...
if __name__ == "__main__":
    ...
    files = sc.textFile(pathfile)
    # get the dates and group the paths by the dates
    grouped_files = files.map(lambda x: (getday(x), x))\
                    .groupByKey()
